...
interface ReturnMetrics {
  ...
  cocoreswapReturn: number // netReturn - hodlReturn
  impLoss: number
  fees: number
}
...
export function getMetricsForPositionWindow(position0: LPSnapshots, position1: LPSnapshots): ReturnMetrics {
  ...
  const cocoreswap_return = difference_fees_usd + imp_loss_usd
  ...
  return {
    ...
    cocoreswapReturn: cocoreswap_return,
...
export async function getLPReturnsOnPair(
  ...
  for (const index in snapshots) {
    ...
    cocoreswapReturn = cocoreswapReturn + results.cocoreswapReturn

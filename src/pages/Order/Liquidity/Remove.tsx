...
import { FONT, StyledInternalLink, TYPE } from '../../../theme'
//import AppBody from '../AppBody'
import { ClickableText, MaxButton } from './styleds'

const DEFAULT_REMOVE_LIQUIDITY_SLIPPAGE_TOLERANCE = new Percent(5, 100)

export default function RemoveLiquidity({ currency }: { currency: (string | undefined)[] | undefined }) {
  ...
  const handleSelectCurrencyA = useCallback(
    (currency: Currency) => {
      if (currencyIdB && currencyId(currency) === currencyIdB) {
        history.push(`/order?mode=remove&currencyIdA=${currencyId(currency)}&currencyIdB=${currencyIdA}`)
      } else {
        history.push(`/order?mode=remove&currencyIdA=${currencyId(currency)}&currencyIdB=${currencyIdB}`)
      }
    },
  ...
  const handleSelectCurrencyB = useCallback(
    (currency: Currency) => {
      if (currencyIdA && currencyId(currency) === currencyIdA) {
        history.push(`/order?mode=remove&currencyIdA=${currencyIdB}&currencyIdB=${currencyId(currency)}`)
      } else {
        history.push(`/order?mode=remove&currencyIdA=${currencyIdA}&currencyIdB=${currencyId(currency)}`)
      }
    },
    [currencyIdA, currencyIdB, history]
  )
                  ...
                  <RowBetween style={{ justifyContent: 'flex-end' }}>
                    {oneCurrencyIsETH ? (
                      <StyledInternalLink
                        ...
                        to={`/order?mode=remove&currencyIdA=${
                          currencyA?.isNative ? WRAPPED_NATIVE_CURRENCY[chainId].address : currencyIdA
                        }&currencyIdB=${currencyB?.isNative ? WRAPPED_NATIVE_CURRENCY[chainId].address : currencyIdB}`}
                      >
                        Receive WETH
                      </StyledInternalLink>
                    ) : oneCurrencyIsWETH ? (
                      <StyledInternalLink
                        ...
                        }&currencyIdB=${currencyB?.equals(WRAPPED_NATIVE_CURRENCY[chainId]) ? 'ETH' : currencyIdB}`}
                      >
                        Receive ETH
                      </StyledInternalLink>
                    ) : null}
